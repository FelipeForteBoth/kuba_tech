...
function publicUser(user, modulos = []) {
  return {
    ...
    trocarSenha: Boolean(user.must_change_password),
    inatividadeMinutos: INACTIVITY_MINUTES,
    modulos,
  };
}
...
async function refresh(req, res) {
  ...
  if (!user) throw new AppError('Usuário não encontrado.', 404);
  res.json({
    token: signToken({ id: user.id, role: user.role, tenant_id: user.tenant_id }),
    usuario: publicUser(user, await tenantModuleCodes(user.tenant_id)),
  });
}
